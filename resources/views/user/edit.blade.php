        ...
        <div class="card">
            ...
            <div class="card-body">
                <form id="editUserForm" action="{{ route('user.update', $user->id) }}" method="post">
                            ...
                            <select class="form-select select2" name="instansi_id" data-placeholder="Pilih Nama Unit">
                                ...
                                @foreach ($mitras as $mitra)
                                    <option value="{{ $mitra->id }}"
                                        {{ $mitra->id == old('instansi_id', $user->instansi_id) ? 'selected' : '' }}>
                    ...
                    <button type="submit" class="btn btn-success w-100 mt-4 rounded-2">Simpan Perubahan</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            document.getElementById('editUserForm').addEventListener('submit', function(event) {
                ...
                // Mengirim permintaan AJAX untuk memperbarui pengguna
                fetch('{{ route('user.update', $user->id) }}', {
                    ...
                    .then(data => {
                        // Menampilkan SweetAlert jika pembaruan berhasil
                        Swal.fire({
                            position: "top-end",
                            icon: "success",
                            ...
                            window.location.href = data
                                .redirect; // Alihkan ke halaman yang dituju
                        });
                    })
                    .catch(error => {
                        if (error.status === 422) {
                            return error.json().then(data => {
                                // Menampilkan SweetAlert jika pembaruan gagal
                                Swal.fire({
                                    icon: "error",
